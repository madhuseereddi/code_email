...
const { TextEncoder } = require('util'); // Fix for TextEncoder issue
...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: 'wdjn psjv ssba woxz', // Replace with your App Password (not regular Gmail password)
  },
  tls: {
...
agenda.define('send daily email', async () => {
  console.log('Agenda job triggered at:', new Date());
  sendEmail();
});
...
(async function () {
  ...
  // Schedule the job to run daily at 7:23 PM
  await agenda.every('00 05 * * *', 'send daily email', {}, { timezone: 'Asia/Kolkata' });

  console.log('Agenda scheduling setup complete.');
})();

// Start the Express server
app.get('/', (req, res) => {
  res.send('Email scheduler is running with Agenda. Check your logs for email status.');
});
